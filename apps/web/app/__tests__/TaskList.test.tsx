...
describe('TaskList', () => {
  ...
  it('calls setPriorityAction with correct task id and priority value', async () => {
    mockedUseQuery.mockReturnValue({
      data: [{ id: '321', title: 'Task with Priority', completed: false, priority: 1 }],
      isLoading: false,
    });
    const mockCreate = jest.fn();
    const mockUpdateStatus = jest.fn();
    ...
    render(
      <TaskList
        ...
        updateTaskTitleAction={mockUpdateTitle}
        setDueDateAction={mockSetDueDate}
        setPriorityAction={mockSetPriority}
      />
    );

    // Find the priority select for the task
    const prioritySelect = screen.getByTestId('set-priority-select');
    // Simulate changing the priority
    fireEvent.change(prioritySelect, { target: { value: '3' } });
    // Assert the mock was called with correct arguments
    expect(mockSetPriority).toHaveBeenCalledTimes(1);
    const formData = mockSetPriority.mock.calls[0][0];
    expect(formData.get('id')).toBe('321');
    expect(formData.get('priority')).toBe('3');
  });
});
